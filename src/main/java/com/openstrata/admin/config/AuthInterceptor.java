package com.openstrata.admin.config;
 ...
 *
 * <p>In dev-mode (single-tenant starter, auth-contract.md §6) a fixed
 * `tenant_id=local` / `platform-admin` token is minted and RLS still applies.
 *
 * <p>Otherwise the tenant is resolved from the `X-Tenant-Id` gateway header, or
 * from the `tenant_id` / `roles` claims of the bearer JWT (claims are base64-
 * decoded only — signature verification is the gateway's responsibility per
 * auth-contract.md §2). Requests without an identity are rejected with 401.
 */
@Component
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        if (props.getFeatures().isDevMode()) {
    ...
    private Claims decodeClaims(String jwt) {
        try {
            ...
            // minimal claim extraction (no verification — gateway verifies signature)
