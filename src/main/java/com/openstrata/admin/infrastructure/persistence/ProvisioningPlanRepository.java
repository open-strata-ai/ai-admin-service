...
public interface ProvisioningPlanRepository extends JpaRepository<ProvisioningPlanEntity, String> {
    ...
    @Query("select p from ProvisioningPlanEntity p where p.tenantId = ?1 and p.status <> com.openstrata.admin.domain.model.ProvisioningStatus.DONE and p.status <> com.openstrata.admin.domain.model.ProvisioningStatus.FAILED")
